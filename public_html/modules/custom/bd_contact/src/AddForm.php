...
class AddForm implements FormInterface { 
	    function getFormID() { return 'bd_contact_add'; }

		function buildForm(array $form, array &$form_state) { 
			$form['name'] = array( '#type' => 'textfield', '#title' => t('Name'), 
				); 
			$form['message'] = array( '#type' => 'textarea', '#title' => t('Message'), 
				); 
			$form['actions'] = array('#type' => 'actions'); $form['actions']['submit'] = array( '#type' => 'submit', '#value' => t('Add'), ); 
			return $form; 
		}
		...
		{ 
			...
			$form_state['redirect'] = 'admin/content/bd_contact'; return; 
		} 
} 